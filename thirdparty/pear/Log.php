<?php
/**
 * $Header: /repository/pear/Log/Log.php,v 1.66 2007/12/12 06:43:57 jon Exp $
...
class Log
{
    ...
    function _getBacktraceVars($depth)
    {
        ...
        for ( $i = count(@$backtrace) -1 ; $i > 2 ; $i-- ) {
          $parts = explode ( PATH_SEP, @$backtrace[$i]['file']);
          $filename = $parts [ count($parts) -1] . ':' . @$backtrace[$i]['line'] . ':';
          $functions .= $filename. @$backtrace[$i]['function'] . '()->';
        }

        /* Return a 4-tuple containing (file, line, function). */
        return array($file, $line, $func, $functions);
    }
